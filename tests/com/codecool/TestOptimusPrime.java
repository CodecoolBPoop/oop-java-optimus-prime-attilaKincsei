...
class TestOptimusPrime {
    ...
    void numberOfPrimes_AssertEqual_True(int largestElement) {
        ...
        int actual = optimusPrime.numberOfPrimesLimitedCPU(largestElement);
        assertEquals(expected, actual);
    }

    @ParameterizedTest
    @ValueSource(ints = {1_000_000, 10_000_000})
    void numberOfPrimes_PerformanceTest(int largestElement) {
        OptimusPrime optimusPrime = new OptimusPrime();
        assertTimeout(Duration.ofMillis(2000), () -> optimusPrime.numberOfPrimesLimitedCPU(largestElement), "Duration exeeds 2 seconds");
    }
